...
#include "sdkconfig.h"
#define TMP36_CHANNEL ADC_CHANNEL_0  // GPIO1
#define TAG "TMP36"
...
{
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(900000)); // 15 minutes

        if (device_connected) {
            char buf[16];
            int len = snprintf(buf, sizeof(buf), "%.2f", current_temperature);
            ...
            ESP_LOGI(GATTS_TAG, "Sent BLE update: %s", buf);
        }
    }
}
...
void adc_task(void *arg)
{
    ...
    adc_oneshot_config_channel(adc_handle, TMP36_CHANNEL, &chan_cfg);
    ...
    while (1) {
        ...
        current_temperature = (voltage - 0.5) * 100.0;

        ESP_LOGI("TMP36", "Temp=%.2f°C", current_temperature);
